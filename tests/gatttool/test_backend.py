...
class GATTToolBackendTests(unittest.TestCase):
    def setUp(self):
        self.patchers = []
        self.patchers.append(patch("pygatt.backends.gatttool.gatttool.pexpect.spawn"))
        self.spawn = self.patchers[0].start()
        self.spawn.return_value.isalive.return_value = False
        self.spawn.return_value.read_nonblocking.side_effect = pexpect.EOF(value=None)
        self.patchers.append(patch("pygatt.backends.gatttool.gatttool.subprocess"))
    ...
    def test_disconnect_callback(self):
        ...
        assert (
            mock_callback
            in device._backend._receiver._event_vector["disconnected"]["callback"]
        )
        ...
        assert (
            mock_callback
            not in device._backend._receiver._event_vector["disconnected"]["callback"]
        )
        assert (
            len(device._backend._receiver._event_vector["disconnected"]["callback"]) > 0
        )
    ...
    def test_single_byte_notification(self):
        event = {"after": "Notification handle = 0x0024 value: 64".encode("utf8")}
        address = "11:22:33:44:55:66"
        device = self.backend.connect(address)
    ...
    def test_multi_byte_notification(self):
        event = {
            "after": ("Notification handle = 0x0024 value: 64 46 72".encode("utf8"))
        }
        address = "11:22:33:44:55:66"
        device = self.backend.connect(address)
        device.receive_notification = MagicMock()
        ...
        assert 0x24 == device.receive_notification.call_args[0][0]
        assert (
            bytearray([0x64, 0x46, 0x72]) == device.receive_notification.call_args[0][1]
        )

    def test_empty_notification(self):
        event = {"after": "Notification handle = 0x0024 value: ".encode("utf8")}
        address = "11:22:33:44:55:66"
        device = self.backend.connect(address)
        device.receive_notification = MagicMock()
        device._backend._handle_notification_string(event)
    ...
    def test_indication(self):
        event = {"after": "Indication   handle = 0x0024 value: 64".encode("utf8")}
        address = "11:22:33:44:55:66"
        device = self.backend.connect(address)
        device.receive_notification = MagicMock()
        device._backend._handle_notification_string(event)
