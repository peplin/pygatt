...
class BGAPIDeviceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.backend = BGAPIBackend(serial_port=self.mock_device.serial_port_name)
    ...
    def test_char_read(self):
        ...
        self.mock_device.stage_char_read_packets(handle_char, 0x00, expected_value)
        value = device.char_read(UUID(uuid_char))
        assert bytearray(expected_value) == value

        # Test ignore of packet with wrong handle
        expected_value = [0xBE, 0xEF, 0x15, 0xF0, 0x0D]
        self.mock_device.stage_char_read_packets(0, 0x00, expected_value)
        self.mock_device.stage_char_read_packets(handle_char, 0x00, expected_value)
        value = device.char_read(UUID(uuid_char))
        assert bytearray(expected_value) == value
    ...
    def test_discover_characteristics(self):
        ...
        assert (
            characteristics[uuid_char].descriptors[uuid16_to_uuid(0x2902)]
            == handle_desc
        )
