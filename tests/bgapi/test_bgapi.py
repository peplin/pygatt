...
from pygatt.backends.bgapi.bgapi import bgapi_address_to_hex, MAX_CONNECTION_ATTEMPTS
...
class BGAPIBackendTests(unittest.TestCase):
    def setUp(self):
        ...
        self.backend = BGAPIBackend(
            serial_port=self.mock_device.serial_port_name, receive_queue_timeout=0.001
        )
    ...
    def _connect(self):
        self.mock_device.stage_connect_packets(self.address, ["connected", "completed"])
    ...
    def test_reset_and_reconnect(self):
        ...
        with self.assertRaises(NotConnectedError):
            self.backend.start()
        self.assertEqual(
            MAX_CONNECTION_ATTEMPTS + 1, self.mock_device.mocked_serial.read.call_count
        )
    ...
    def test_scan_and_get_devices_discovered(self):
        ...
        devs = self.backend.scan(timeout=0.5)
        assert "Hello!" == devs[0]["name"]
        assert -80 == devs[0]["rssi"]

    def test_clear_bonds(self):
        # Test delete stored bonds
        self.mock_device.stage_clear_bonds_packets([0x00, 0x01, 0x02, 0x03, 0x04])
