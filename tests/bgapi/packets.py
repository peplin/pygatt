...
class BGAPIPacketBuilder(object):
    @staticmethod
    def attclient_attribute_write(connection_handle, return_code):
        ...
        # class?
        return pack("<4BBH", 0x00, 0x03, 0x04, 0x05, connection_handle, return_code)

    @staticmethod
    def attclient_write_command(connection_handle, return_code):
        return pack("<4BBH", 0x00, 0x03, 0x04, 0x06, connection_handle, return_code)

    @staticmethod
    def attclient_find_information(connection_handle, return_code):
        return pack("<4BBH", 0x00, 0x03, 0x04, 0x03, connection_handle, return_code)

    @staticmethod
    def attclient_read_by_handle(connection_handle, return_code):
        return pack("<4BBH", 0x00, 0x03, 0x04, 0x04, connection_handle, return_code)

    @staticmethod
    def connection_disconnect(connection_handle, return_code):
        return pack("<4BBH", 0x00, 0x03, 0x03, 0x00, connection_handle, return_code)

    @staticmethod
    def connection_get_rssi(connection_handle, rssi_value):
        return pack("<4BBb", 0x00, 0x02, 0x03, 0x01, connection_handle, rssi_value)

    @staticmethod
    def gap_connect_direct(connection_handle, return_code):
        return pack("<4BHB", 0x00, 0x03, 0x06, 0x03, return_code, connection_handle)
    ...
    def sm_encrypt_start(connection_handle, return_code):
        return pack("<4BBH", 0x00, 0x03, 0x05, 0x00, connection_handle, return_code)
    ...
    @staticmethod
    def attclient_attribute_value(connection_handle, att_handle, att_type, value):
    ...
    @staticmethod
    def attclient_procedure_completed(connection_handle, return_code, chr_handle):
        return pack(
            "<4BB2H", 0x80, 0x05, 0x04, 0x01, connection_handle, return_code, chr_handle
        )
    ...
    def connection_disconnected(connection_handle, return_code):
        return pack("<4BBH", 0x80, 0x03, 0x03, 0x04, connection_handle, return_code)

    @staticmethod
    def gap_scan_response(rssi, packet_type, bd_addr, addr_type, bond, data):
    ...
    def sm_bond_status(bond_handle, keysize, mitm, keys):
        return pack("<4B4B", 0x80, 0x04, 0x05, 0x04, bond_handle, keysize, mitm, keys)

    @staticmethod
    def sm_bonding_fail(connection_handle, return_code):
        return pack("<4BBH", 0x80, 0x03, 0x05, 0x01, connection_handle, return_code)
