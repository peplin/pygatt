...
class BGAPICommandPacketBuilder(object):
    ...
    def system_endpoint_tx(endpoint, data):
        return pack(
            "<4BBB%dB" % len(data), 0, 2 + len(data), 0, 9, endpoint, len(data), *data
        )
    ...
    def flash_ps_save(key, value):
        return pack(
            "<4BHB%dB" % len(value), 0, 3 + len(value), 1, 3, key, len(value), *value
        )
    ...
    def connection_raw_tx(connection, data):
        return pack(
            "<4BBB%dB" % len(data), 0, 2 + len(data), 3, 8, connection, len(data), *data
        )

    @staticmethod
    def attclient_find_by_type_value(connection, start, end, uuid, value):
        return pack(
            "<4BBHHHB%dB" % len(value),
            0,
            8 + len(value),
            4,
            0,
    ...
    def gap_set_filtering(scan_policy, adv_policy, scan_duplicate_filtering):
        return pack(
            "<4BBBB", 0, 3, 6, 6, scan_policy, adv_policy, scan_duplicate_filtering
        )
    ...
    def gap_set_adv_parameters(adv_interval_min, adv_interval_max, adv_channels):
        return pack(
            "<4BHHB", 0, 5, 6, 8, adv_interval_min, adv_interval_max, adv_channels
        )

    @staticmethod
    def gap_set_adv_data(set_scanrsp, adv_data):
        return pack(
            "<4BBB%dB" % len(adv_data),
            0,
            2 + len(adv_data),
            6,
            9,
    ...
    def hardware_spi_config(channel, polarity, phase, bit_order, baud_e, baud_m):
        return pack(
            "<4BBBBBBB", 0, 6, 7, 8, channel, polarity, phase, bit_order, baud_e, baud_m
        )

    @staticmethod
    def hardware_spi_transfer(channel, data):
        return pack(
            "<4BBB%dB" % len(data), 0, 2 + len(data), 7, 9, channel, len(data), *data
        )
